...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [WIN_BASE] = LAYOUT_ansi_108(
        KC_ESC,                  KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,     KC_F12,     KC_PSCR,  KC_MS_BTN1, UG_TOGG,  KC_MUTE,  KC_VOLD,  KC_VOLU,  KC_MPLY,
    ...
    [WIN_FN] = LAYOUT_ansi_108(
        _______,                 KC_BRID,  KC_BRIU,  KC_TASK,  KC_FILE,  UG_VALD,  UG_VALU,  KC_MPRV,  KC_MPLY,  KC_MNXT,  KC_MUTE,  KC_VOLD,    KC_VOLU,    _______,  _______,    _______,  KC_MPRV,  KC_MNXT,  KC_MSEL,  KC_MSTP,
...
void os_state_indicate(void) {
    // Mantém a proteção de sleep, mas sem depender de headers internos:
    // (se você precisar mesmo dessa proteção, dá pra incluir os headers do Keychron,
    // mas normalmente pode omitir sem problemas)
    //
    // NÃO coloque o bloco de NUM_LOCK aqui.

#   if defined(CAPS_LOCK_INDEX)
    if (host_keyboard_led_state().caps_lock) {
#       if defined(DIM_CAPS_LOCK)
        // No firmware da Keychron isso costuma significar "dim" invertendo ON/OFF
